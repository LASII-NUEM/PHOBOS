...
def rosenbrock_residue(theta):
    points.append(theta)
    first_half = 1-theta[0]
    second_half = 10*(theta[1]-theta[0]**2)

    return np.array([first_half,second_half]) #rosenbrock function
...
#run BFGS
theta = least_squares(rosenbrock_residue, [-1.2,1], method='lm')
...
def animate(i):
    ...
    leg.append('Non-Linear Least-Squares')
...
ani.save("./NLLS.gif", dpi=300, writer=PillowWriter(fps=25))
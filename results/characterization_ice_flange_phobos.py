...
spec_ice_obj.Cp = np.mean(spec_ice_obj.Cp[0,:,:], axis=1)
spec_ice_obj.Rp = np.mean(spec_ice_obj.Rp[0,:,:], axis=1)
...
# spec_ice_obj.Cp = spec_ice_obj.Cp[0,:,:]
# spec_ice_obj.Rp = spec_ice_obj.Rp[0,:,:]

#dielectric parameters
exp_eps_real, exp_eps_imag = characterization_utils.dielectric_params_generic(spec_ice_obj, spec_air_obj, spec_ice_obj.freqs) #compute the spectrum based on the experimental data
ideal_eps_real, ideal_eps_imag = characterization_utils.dielectric_params_Artemov2013(spec_ice_obj.freqs, medium="ice")

plt.figure(1)
plt.plot(np.log10(spec_ice_obj.freqs), exp_eps_real, label="ε' meas.")
plt.plot(np.log10(spec_ice_obj.freqs), exp_eps_imag, label="ε'' meas.")
plt.plot(np.log10(spec_ice_obj.freqs), ideal_eps_real, label="ε' Artemov", linestyle='dotted', color='black')
plt.plot(np.log10(spec_ice_obj.freqs), ideal_eps_imag, label="ε'' Artemov", linestyle='dashed', color='black')
ax = plt.gca()
